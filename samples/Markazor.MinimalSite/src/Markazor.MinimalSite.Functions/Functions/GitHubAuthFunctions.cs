...
namespace Markazor.MinimalSite.Functions.Functions;
...
internal sealed class GitHubAuthFunctions(MarkazorFunctionsEndpointService endpoints)
{
    ...
    public HttpResponseData Start(
        [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "auth/github/start")] HttpRequestData request)
    {
        return endpoints.StartGitHubAuth(request);
    }

    [Function("GitHubAuthCallback")]
    public Task<HttpResponseData> CallbackAsync(
        [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "auth/github/callback")] HttpRequestData request,
    ...
    public Task<HttpResponseData> RefreshAsync(
        [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "auth/github/refresh")] HttpRequestData request,
        CancellationToken cancellationToken)
    {
